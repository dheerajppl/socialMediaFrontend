...
import { useEditProfileMutation } from '@/service';

const EditProfile = () => {
    const imageRef = useRef();
    const { user } = useSelector(store => store.auth);
    const [reqEditProfile, resEditProfile] = useEditProfileMutation();
    ...
    useEffect(() => {
        ...
        if(resEditProfile?.isError){
            toast.error(resEditProfile.error.message);
        }
    ...
    const editProfileHandler = async () => {
        ...
        reqEditProfile(formData);
        // try {
        //     setLoading(true);
        //     const res = await axios.post('http://localhost:5097/api/user/profile/edit', formData,{
        //         headers:{
